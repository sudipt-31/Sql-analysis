    ...
    if st.button("Execute Query"):
        ...
        with st.spinner("Processing question and updating database..."):
            try:
                ...
                # Add new topics to the matching DataFrame only for single sheet cases
                if not getattr(st.session_state, 'is_multi_sheet', False):
                    if st.session_state.matching_df is not None:
                        ...
                        # Update database for single sheet case
                ...
                # Get schema information with sample data
                ...
                for table in tables:
                    table_name = table[0]
                    # Skip subtopic_matching table for multi-sheet cases
                    if getattr(st.session_state, 'is_multi_sheet', False) and table_name == 'subtopic_matching':
                        continue
                ...
                # Modify prompt based on whether it's multi-sheet or single-sheet
                if getattr(st.session_state, 'is_multi_sheet', False):
                    prompt = f"""
                    ...
                    Do NOT use the subtopic_matching table in your query.
                    Consider using JOINs between tables if needed to answer the question.
