                    ...
                    for sheet_name, sheet_df in valid_sheets.items():
                        if not sheet_df.empty and len(sheet_df.columns) > 0:
                            ...
                            try:
                                ...
                                # Store schema information for this table
                                schema_info[table_name] = {
                                    'columns': list(sheet_df.columns),
                                    'sample_data': sheet_df.head(1).to_dict('records')[0] if not sheet_df.empty else {}
                                }
    ...
    if st.button("Execute Query"):
        ...
        with st.spinner("Processing question and updating database..."):
            try:
                ...
                # Get schema information based on whether it's multi-sheet or single sheet
                if st.session_state.is_multi_sheet:
                    # For multi-sheet files, use the stored schema information
                    schema_str = "Available tables and their columns:\n\n"
                    for table_name, info in st.session_state.db_schema.items():
                        schema_str += f"Table: {table_name}\n"
                        schema_str += "Columns:\n"
                        for col in info['columns']:
                            sample_val = info['sample_data'].get(col, 'NULL')
                            schema_str += f"- {col} (TEXT) - Sample: {sample_val}\n"
                        schema_str += "\n"
                else:
                    # For single sheet files, use the existing schema generation logic
                ...
                For multi-sheet queries, make sure to reference the correct table name.
                ...
                Note: This is a {'multi-sheet' if st.session_state.is_multi_sheet else 'single-sheet'} database.
                """
